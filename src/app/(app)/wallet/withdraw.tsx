...
export default function WithdrawScreen() {
  ...
  const handleSubmit = async () => {
    if (!kycVerified) {
      Alert.alert("KYC Required", "Please complete KYC verification in Settings before withdrawing.");
      return;
    }
    ...
    if (!selectedMethodId) {
      Alert.alert("Payment Method", "Please select a verified bank account or UPI ID.");
      return;
    }
  ...
  const renderWithdrawal = ({ item }: { item: Withdrawal }) => {
    ...
    return (
      <View className="bg-surface-card border border-brand-primary/5 rounded-xl p-4 mb-3">
        ...
        {item.status === "failed" && item.failure_reason && (
          <Text className="text-danger-400 text-xs mt-1">{item.failure_reason}</Text>
        )}
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View className="px-4">
            {/* KYC Warning */}
            {!kycVerified && (
              <Card style={{ marginBottom: 16, borderColor: "rgba(245,158,11,0.3)" }}>
                <View className="p-4 flex-row items-center">
                  <Ionicons name="warning-outline" size={20} color="#f59e0b" />
                  <View className="ml-3 flex-1">
                    <Text className="text-warning-400 font-bold text-sm">KYC Verification Required</Text>
            ...
            {kycVerified && (
              <Card style={{ marginBottom: 16 }}>
                <View className="p-5">
                  <Text className="text-white font-bold text-sm mb-3">Withdraw Amount</Text>
                  <AmountInput
                    label="Enter Amount"
                    valuePaise={amount}
                  ...
                  <View className="flex-row gap-2 mb-4">
                    {[1000_00, 5000_00, 10000_00].map((amt) => (
                      <TouchableOpacity
                        ...
                        className="bg-brand-500/10 border border-brand-500/20 py-1.5 px-3 rounded-lg flex-1"
                      >
                        <Text className="text-brand-500 text-xs font-bold text-center">
                  ...
                  {loadingMethods ? (
                    <View className="flex-row items-center justify-center py-4">
                      ...
                      <Text className="text-neutral-400 text-xs ml-2">Loading payment methods...</Text>
                    </View>
                  ) : paymentMethods.length === 0 ? (
                    <View className="bg-surface-950 rounded-xl p-4 mb-4 border border-surface-border">
                      <View className="flex-row items-center">
                        <Ionicons name="card-outline" size={18} color="#a3a3a3" />
                        <View className="ml-3 flex-1">
                          <Text className="text-neutral-300 text-xs font-semibold">No verified payment methods</Text>
                    ...
                    <View className="mb-4">
                      <Text className="text-neutral-300 text-xs font-semibold mb-2 ml-1">
                        Send to
                      </Text>
                        ...
                        <TouchableOpacity
                          ...
                          className={`flex-row items-center p-3 rounded-xl mb-2 border ${
                            selectedMethodId === method.id
                              ...
                              : "border-surface-border bg-surface-950"
                          }`}
                        >
                          <View className="w-8 h-8 rounded-full items-center justify-center bg-surface-card">
                            <Ionicons
                              ...
                              size={14}
                              color={selectedMethodId === method.id ? COLORS.brandPrimary : "#a3a3a3"}
                            />
                          </View>
                          <View className="ml-3 flex-1">
            ...
            {activeWithdrawals.length > 0 && (
              <View className="mb-4">
                <Text className="text-white text-base font-bold mb-3">Active Withdrawals</Text>
            ...
            {pastWithdrawals.length > 0 && (
              <View className="mb-4">
                <Text className="text-white text-base font-bold mb-3">Withdrawal History</Text>
